    ...
    useEffect(() => {
        ...
        const fetchDynamicBlog = async () => {
            try {
                ...
                const listRes = await axios.get("http://192.168.0.141:8000/blogs/", {
                ...
                const detailRes = await axios.get(`http://192.168.0.141:8000/blogs/${blogId}`, {
                    headers: { "Authorization": `Bearer ${token}` }
                });
                ...
                let heroData = {};
                try {
                    const heroRes = await axios.get(`http://192.168.0.141:8000/blogs/${blogId}/hero`, {
                        headers: { "Authorization": `Bearer ${token}` }
                    });
                ...
                for (let id = 1; id <= maxScanId; id++) {
                    sectionPromises.push(
                        axios.get(`http://192.168.0.141:8000/blogs/sections/${id}`, {
                            headers: { "Authorization": `Bearer ${token}` }
                        })
                            .then(res => {
                ...
                if (authorAvatarPath) {
                    authorAvatarUrl = (authorAvatarPath.startsWith("http://") || authorAvatarPath.startsWith("https://"))
                        ? authorAvatarPath
                        : `http://192.168.0.141:8000${authorAvatarPath}`;
                }
                            ...
                            blog.sections.map((section, idx) => {
                                ...
                                if (imagePath) {
                                    imageUrl = (imagePath.startsWith("http://") || imagePath.startsWith("https://"))
                                        ? imagePath
                                        : `http://192.168.0.141:8000${imagePath}`;
                                }
